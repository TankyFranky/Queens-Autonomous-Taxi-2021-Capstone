...
def clbk_laser(msg):
    ...
    global ir, il
    regions_ = {
            'r1':   min(min(msg.ranges[415:449]), 10),
            'r2':   min(min(msg.ranges[378:414]), 10),
            'r3':   min(min(msg.ranges[343:377]), 10),
            'r4':   min(min(msg.ranges[306:342]), 10),
            'r5':   min(min(msg.ranges[270:305]), 10),
            'l5':   min(min(msg.ranges[235:269]), 10),
            'l4':   min(min(msg.ranges[198:234]), 10),
            'l3':   min(min(msg.ranges[162:197]), 10),
            'l2':   min(min(msg.ranges[126:161]), 10),
            'l1':   min(min(msg.ranges[89:125]), 10),
            'right':  min(min(msg.ranges[377:449]), 10),
            'fright': min(min(msg.ranges[305:376]), 10),
            'front':  min(min(msg.ranges[233:304]), 10),
            'fleft':  min(min(msg.ranges[161:232]), 10),
            'left':   min(min(msg.ranges[89:160]), 10),
            'r1r':	min(min(msg.ranges[439:449]), 10),
            'r2r':	min(min(msg.ranges[369:379]), 10),
            'l1l':	min(min(msg.ranges[89:109]), 10),
            'l2l':	min(min(msg.ranges[169:179]), 10),
            #assuming 721 = pi
    }
...
def take_action():
    ...
    global regions_, idist, Aref_r, Aref_l, fku, idist_s 
    fku = 1
    ...
    if abs(regions_['l2l'] - regions_['l1l']) > 3:
    	Aref_l = 11.0
    else:
    ...
    if Aref_l < 10 and Aref_r < 10:
    	idist = regions_['l1l'] + regions_['r1r']
        fku = 0
...
def followleft():
    ...
    angular_z = 0
    state_description = ''
    d = idist/3#safety distance

    if regions['left'] > d:
        if regions['left'] < d*2 and regions['front'] > idist:
            state_description = 'case 1 -forward'
            change_state(1)
        elif regions['left'] > d*2 and regions['fleft'] > idist:
            state_description = 'case 2 -turnleft'
            change_state(3)
        elif regions['front'] < idist and regions['left'] < idist:
        ...
        if regions['right'] > d and regions['front'] < d:
            state_description = 'case 3 -turnright'
            change_state(2)
...
def followright():
    ...
    if regions['right'] > d:
        if regions['right'] < d*2 and regions['front'] > idist:
            state_description = 'case 1 -forward'
            change_state(1)
        elif regions['right'] > d*2 and regions['fright'] > idist:
            state_description = 'case 2 -turnright'
            change_state(2)
        elif regions['front'] < idist and regions['right'] < idist:
        ...
        if regions['left'] > d and regions['front'] < d:
            state_description = 'case 3 -turnleft'
            change_state(3)
